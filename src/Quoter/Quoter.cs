...
public class Quoter {
	...
	private readonly static HashSet<string> factoryMethodsToExclude = new HashSet<string> {
	...
	private readonly static Dictionary<string, IEnumerable<MethodInfo>> factoryMethods = GetFactoryMethods();
	...
	/// </summary>
	private readonly static Dictionary<string, PropertyInfo> triviaFactoryProperties = GetTriviaFactoryProperties();
	/// <summary>
	...
	/// </summary>
	private readonly static string[] nonStructuralProperties = {
		"AllowsAnyExpression",
		"Arity",
		"ContainsAnnotations",
		"ContainsDiagnostics",
		...
		foreach ( var factoryMethodParameter in factory.GetParameters() ) {
			...
			if ( quotedCodeBlock != null && factory.GetParameters().Length == 1 && factoryMethodParameter.GetCustomAttribute<ParamArrayAttribute>() != null ) {
				...
				if ( methodCall != null && methodCall.Name.Contains( "List" ) && methodCall.Arguments.Count == 1 ) {
					var argument = methodCall.Arguments[index: 0] as ApiCall;
			...
			if ( parameterName == "name" && parameterType == typeof( string ) ) {
				...
				if ( quotedCodeBlock == null ) {
					throw new NotImplementedException( $"An unsupported factory method was chosen: {factory}\r\nPlease add this method to the list of factoryMethodsToExclude." );
				}

				var methodCall = quotedCodeBlock.FactoryMethodCall as MethodCall;
				if ( methodCall != null && methodCall.Name == SyntaxFactoryMethod( "Identifier" ) ) {
					if ( methodCall.Arguments.Count == 1 ) {
						factoryMethodCall.AddArgument( methodCall.Arguments[index: 0] );
					} else {
	...
	/// <param name="normalizeWhitespace"></param>
	...
	private static string GetPrintableTypeName( Type parameterType ) {
		...
		var sb = new StringBuilder();
		sb.Append( parameterType.Name.Substring( startIndex: 0, length: parameterType.Name.IndexOf( value: '`' ) ) );
		sb.Append( "<" );
	...
	private static Dictionary<string, PropertyInfo> GetTriviaFactoryProperties() {
		var result = typeof( SyntaxFactory )
			...
			.ToDictionary( propertyInfo => ( (SyntaxTrivia) propertyInfo.GetValue( obj: null ) ).ToString() );
	...
	/// </summary>
	private static MethodInfo PickFactoryMethodToCreateNode( SyntaxNode node ) {
		...
		var literalExpressionSyntax = node as LiteralExpressionSyntax;
		if ( literalExpressionSyntax != null ) {
			var kind = literalExpressionSyntax.Kind();
	...
	private string Print( ApiCall root ) {
		var sb = new StringBuilder();
		Print( root, sb, depth: 0, openParenthesisOnNewLine: OpenParenthesisOnNewLine, closingParenthesisOnNewLine: ClosingParenthesisOnNewLine );
	...
	private static void Print(
		...
		if ( methodCall != null ) {
			if ( methodCall.Arguments == null || !methodCall.Arguments.Any() ) {
				Print( openParen + closeParen, sb, indent: 0 );
				return;
			}
			...
			if ( methodCall.Arguments.Count == 1 &&
				( methodCall.Arguments[index: 0] is string || methodCall.Arguments[index: 0] is SyntaxKind ) ) {
				needNewLine = false;
			}
	...
	private static void PrintIndent( StringBuilder sb, int indent ) {
		if ( indent > 0 ) {
			sb.Append( new string( c: ' ', count: indent * 4 ) );
		}
	}
	...
	private static string ProperCase( string str ) {
		return char.ToUpperInvariant( str[index: 0] ) + str.Substring( startIndex: 1 );
	}
	...
	private ApiCall QuoteList( IEnumerable syntaxList, string name ) {
		...
		var methodName = SyntaxFactoryMethod( "List" );
		...
		if ( propertyType.IsGenericType ) {
			...
			if ( propertyType.GetGenericTypeDefinition() == typeof( SeparatedSyntaxList<> ) ) {
				...
				sourceList = ( (SyntaxNodeOrTokenList)
						syntaxList.GetType().GetMethod( "GetWithSeparators" ).Invoke( syntaxList, parameters: null ) )
					.Cast<object>()
					.ToArray();
			}
	...
	private ApiCall QuotePropertyValue( SyntaxNode node, PropertyInfo property ) {
		var value = property.GetValue( node, index: null );
	...
	private ApiCall QuoteToken( SyntaxToken value, string name ) {
		...
		var methodName = SyntaxFactoryMethod( "Token" );
		var verbatim =
		...
		if ( value.Kind() == SyntaxKind.IdentifierToken && !value.IsMissing ) {
			...
			var escapedValue = EscapeAndQuote( value.ValueText );
	...
	private ApiCall QuoteTrivia( SyntaxTrivia syntaxTrivia ) {
		var factoryMethodName = SyntaxFactoryMethod( "Trivia" );
		...
		if ( triviaFactoryProperties.TryGetValue( syntaxTrivia.ToString(), out triviaFactoryProperty ) &&
			( (SyntaxTrivia) triviaFactoryProperty.GetValue( obj: null ) ).Kind() == syntaxTrivia.Kind() ) {
			if ( UseDefaultFormatting ) {
				return null;
			}

			return new ApiCall( parentPropertyName: null, factoryMethodName: SyntaxFactoryMethod( triviaFactoryProperty.Name ) );
		}
		...
		return new ApiCall( parentPropertyName: null, factoryMethodName: factoryMethodName, arguments: CreateArgumentList( argument ) );
	}
	...
	private class ApiCall {
		...
		public MemberCall FactoryMethodCall { get; }
		...
		public bool UseCurliesInsteadOfParentheses { get; }
